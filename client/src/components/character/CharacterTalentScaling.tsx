...
const CharacterTalentScaling: React.FC<CharacterTalentScalingProps> = ({talent}) => {
    ...
    return (<div>
        <Table>
            {talent.scaling.map((s) => {
                return <TableRow>
                    <TableHead>{s.key}</TableHead>
                    <TableCell>{s.value[Math.min(level - 1, s.value.length - 1)]}</TableCell>
                </TableRow>
            })}
        </Table>
        <div className="w-full max-w-md mx-auto p-4">
            <Slider step={1} max={maxCount} min={1} onValueChange={handleChange} value={[level]}
                    className="transition-all duration-300 ease-in-out"/>
            <div className="mt-2 text-center">
                <span className="text-lg font-semibold">Level: {level} / {maxCount}</span>
            </div>
        </div>
    </div>);
}
