...
const locations = ["Mondstadt", "Liyue", "Inazuma", "Sumeru", "Fontaine", "Natlan"] as const;
...
async function findTalentsForRegion(region: Location, driver: WebDriver): Promise<Talent[]> {
    ...
    for (const row of rows) {
        ...
        const bookImages = await cells[1].findElement(By.css("span.card-list-container"))
            .findElements((By.css("span.card-image-container img")));

        const books = await Promise.all(bookImages.map(async (book) => {
            const name = await book.getAttribute("alt");
            const url = await book.getAttribute("data-src");
            return {name, url: parseUrl(url)};
        }));

        const characterContainers = await cells[2].findElements(By.css("span.card-body"));
        const characters = await Promise.all(characterContainers.map(async (container) => {
            const name = await container.findElement(By.css("span.card-text"))
                .getText();
            const url = await container.findElement(By.css(`span.card-image-container img`))
